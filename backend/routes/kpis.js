...
// Endpoint para obtener el listado detallado de egresos/gastos con filtros de fecha
router.get('/gastos-detallados', async (req, res) => {
    ...
    try {
        ...
        if (desde && hasta) {
            conditions.push(`fecha AT TIME ZONE 'America/La_Paz' >= $1::timestamp AND fecha AT TIME ZONE 'America/La_Paz' <= $2::timestamp`);
            params.push(`${desde} 00:00:00`);
            params.push(`${hasta} 23:59:59`);
        } else {
            conditions.push(`EXTRACT(MONTH FROM fecha AT TIME ZONE 'America/La_Paz') = EXTRACT(MONTH FROM CURRENT_TIMESTAMP AT TIME ZONE 'America/La_Paz')`);
            conditions.push(`EXTRACT(YEAR FROM fecha AT TIME ZONE 'America/La_Paz') = EXTRACT(YEAR FROM CURRENT_TIMESTAMP AT TIME ZONE 'America/La_Paz')`);
        }
        ...
        const queryText = `
            SELECT 
                id, 
                TO_CHAR(fecha AT TIME ZONE 'America/La_Paz', 'YYYY-MM-DD HH24:MI:SS') as fecha_formateada,
                fecha,
                monto,
                tipo,
                categoria,
            ...
            FROM (
                SELECT 
                    ...
                    'Compra'::text as tipo,
                    COALESCE(p.nombre, 'Proveedor No Registrado') as categoria,
                    COALESCE(
                    ...
                    CASE WHEN c.total > 200 THEN 'BANCO BISA' ELSE 'EFECTIVO' END as metodo_pago
                FROM compras c
                ...
                SELECT 
                    gc.id,
                    gc.fecha,
                    gc.monto::numeric as monto,
                    'Caja Chica'::text as tipo,
                    'Caja Chica'::text as categoria,
                    gc.descripcion,
                ...
                SELECT 
                    gg.id,
                    gg.fecha,
                    gg.monto::numeric as monto,
                    'General'::text as tipo,
                    gg.categoria,
                    gg.descripcion,
                    gg.metodo_pago
